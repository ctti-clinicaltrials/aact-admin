...
describe Util::FileManager do
  ...
  context 'when removing daily files' do
    it 'does not delete files datestamped with current month/year' do
      dt = Time.zone.now
      current_year = dt.year
      current_month = dt.month.to_s.rjust(2, '0')
      this_month_file="/aact-files/static_db_copies/daily/#{current_year}#{current_month}02_clinical_trials.zip"
    ...
    it 'removes all files other than those from current year/month' do
      dt = Time.zone.now
      year  = dt.year
