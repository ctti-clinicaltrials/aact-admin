...
describe Util::FileManager do
  ...
  context 'when removing daily files' do
    it 'does not delete files datestamped with current year/month' do
      ...
      this_month_file="#{ENV['AACT_STATIC_FILE_DIR']}/static_db_copies/daily/#{year}#{month}02_clinical_trials.zip"
      system("rm #{ENV['AACT_STATIC_FILE_DIR']}/static_db_copies/daily/*") # remove everything from daily file directory
    ...
    it 'removes all files other than those datestamped with current year/month' do
      ...
      last_month_file="#{ENV['AACT_STATIC_FILE_DIR']}/static_db_copies/daily/#{year}#{month}02_clinical_trials.zip"
